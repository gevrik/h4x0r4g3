...
class WebsocketService implements MessageComponentInterface {
    ...
    public function __construct(
        ...
        /* LOAD DB INTO MEMORY */
        $this->serverDataService->loadDataFromDb();
    ...
    {
        return $this->serverDataService->findById($name, $id);
    }
    ...
    {
        return $this->serverDataService->findByPropertyValue($name, $property, $value);
    }
    ...
    {
        return $this->serverDataService->getPropertyValueById($name, $property, $id);
    }
    ...
    {
        return $this->serverDataService->executeDataServiceMethod($methodName, $parameters);
    }
        ...
        try {
            ...
            switch ($command) {
                default:
                    break;
                case 'setipaddy':
                    // make sure to validate the ip addy
                    $validator = new Ip();
                    if ($validator->isValid($content)) {
                        // check if the ip is banned
                        $bannedIpEntry = $this->serverDataService->countByPropertyValue('bannedips', 'ip', $content);
                        if ($bannedIpEntry>=1) {
                            $response = new GameClientResponse($resourceId);
                ...
                case 'processlocations':
                    ...
                    foreach ($content as $locationData) {
                        ...
                        $existingGeocoord = $this->serverDataService->findByLatLngPlace($lat, $lng, $placeId);
                        if (!$existingGeocoord) {
                            $geocoordData = [
                                'id' => NULL,
                                'added' => new \DateTime(),
                            ...
                            $this->serverDataService->addRecord('geocoords', $geocoordData);
                            if ($awaitingcoords) $possibleLocations[] = $geocoordData;
                        }
                        else {
                    ...
                    if ($awaitingcoords) {
                        ...
                        if (!empty($possibleLocations)) {
                            ...
                            $flytoResponse->send();
                            $affectedProfileId = $this->getClientData($resourceId);
                            $affectedSystemId = $this->serverDataService->getPropertyValueById('profiles', 'currentSystemId', $affectedProfileId);
                            $this->serverDataService->updatePropertyValueById('systems', $affectedSystemId, 'geocoords', $location);
                            return $this->utilityService->updateSystemCoords($resourceId, $location);
                        }
                        else {
